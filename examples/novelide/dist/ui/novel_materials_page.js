...
function MaterialsPage(ctx) {
    const { UI } = ctx;
    const colors = ctx.MaterialTheme.colorScheme;
    const params = ctx.routeParams ?? {};
    const workId = params.workId ?? "";
    const [activeTab, setActiveTab] = ctx.useState("activeTab", 0);
    const [items, setItems] = ctx.useState("items", []);
    const [loading, setLoading] = ctx.useState("loading", false);
    const [editingItem, setEditingItem] = ctx.useState("editingItem", null);
    const [showForm, setShowForm] = ctx.useState("showForm", false);
    const [formFields, setFormFields] = ctx.useState("formFields", {});
    const category = CATEGORIES[activeTab];
    async function loadItems() {
        setLoading(true);
        try {
            const result = await window.NativeBridge[category.bridgeGet](workId);
    ...
    async function saveItem() {
        try {
            const payload = editingItem
                ...
                : { ...formFields };
            await window.NativeBridge[category.bridgeSave](workId, JSON.stringify(payload));
            setShowForm(false);
            setEditingItem(null);
            setFormFields({});
            await loadItems();
    ...
    async function deleteItem(itemId) {
        try {
            await window.NativeBridge[category.bridgeDelete](itemId);
            await loadItems();
        }
        catch (error) {
    ...
    const tabBar = UI.Box({
        ...
        UI.LazyColumn({ fillMaxSize: true, contentPadding: 4 }, [
            ...CATEGORIES.map((cat, index) => UI.Card({
    ...
    const formView = showForm
        ? UI.Box({
            ...
            UI.LazyColumn({ fillMaxSize: true, contentPadding: 12, spacing: 12 }, [
                ...category.fields.map((field) => UI.TextField({
                    value: formFields[field.key] || "",
                    onValueChange: (v) => setFormFields({ ...formFields, [field.key]: v }),
                    label: field.label,
                ...
                UI.Row({ fillMaxWidth: true, spacing: 8, padding: 12 }, [
                    ...
                    UI.Button({ onClick: () => setShowForm(false), variant: "outlined" }, "取消"),
                ]),
            ]),
        ])
        : null;
