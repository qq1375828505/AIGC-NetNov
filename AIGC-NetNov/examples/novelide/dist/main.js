...
function buildMessage(systemPrompt, userContent) {
  return `[\u7CFB\u7EDF\u6307\u4EE4] ${systemPrompt}

[\u7528\u6237\u8BF7\u6C42] ${userContent}`;
}
async function callAi(systemPrompt, content) {
  var _a;
  ...
  const aiReply = ((_a = result.aiResponse) != null ? _a : "").trim();
  if (!aiReply) {
    throw new Error("AI \u8FD4\u56DE\u4E86\u7A7A\u56DE\u590D");
  }
  return aiReply;
}
async function callAiStreaming(systemPrompt, content, onChunk) {
  var _a;
  const fullMessage = buildMessage(systemPrompt, content);
  let accumulated = "";
  const result = await Tools.Chat.sendMessageStreaming(fullMessage, void 0, void 0, void 0, {
    onIntermediateResult: (event) => {
      if (event.type === "chunk" && event.chunk) {
        accumulated += event.chunk;
        onChunk == null ? void 0 : onChunk(event.chunk, accumulated);
      }
    }
  });
  if (!result) {
  ...
  const aiReply = ((_a = result.aiResponse) != null ? _a : accumulated).trim();
  if (!aiReply) {
    throw new Error("AI \u8FD4\u56DE\u4E86\u7A7A\u56DE\u590D");
  }
  return aiReply;
}
function registerAiTool(name, description, paramDefs, required, systemPrompt, buildUserContent) {
  Tools.register(`${name}_stream`, {
    ...
    execute: async (params) => {
      ...
      try {
        ...
        sendIntermediateResult({ success: true, streaming: true, text, done: true });
        return { success: true, text };
      } catch (error) {
        const msg = error.message || "AI \u8C03\u7528\u5931\u8D25";
  ...
  Tools.register(name, {
    ...
    execute: async (params) => {
      ...
      try {
        ...
        return { success: false, error: error.message || `${name} \u5931\u8D25` };
      }
    }
  });
}
...
[\u7528\u6237\u8BF7\u6C42] ${fullPrompt}`;
   ...
   * 调用子 Agent（流式）
   */
  static async dispatchStreaming(agentId, task, context, onChunk) {
    var _a;
    const agent = AGENT_CONFIGS[agentId];
    if (!agent) {
      throw new Error(`\u672A\u77E5\u7684 Agent: ${agentId}`);
